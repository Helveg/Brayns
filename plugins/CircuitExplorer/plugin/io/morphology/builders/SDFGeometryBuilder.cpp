...
public:
    ...
    std::vector<SectionType> _sectionTypeMap;
private:
    const Morphology& m;
    ...
    void _addSomaGeometry()
    {
        ...
        if(!soma.children.empty())
        {
            ...
            for (const auto child : soma.children)
            {
                ...
                const size_t geomIdx = _addSDFGeometry(std::move(geometry), -1, SectionType::SOMA);
...
{
   std::unique_ptr<MorphologyInstance> instance =
        std::make_unique<SDFMorphologyInstance>(_sdfGeometries,
                                                _sdfNeighbours,
                                                _sdfSectionTypes,
                                                _sectionMap);
   instance->transform(tr, rot);
   return instance;
}
