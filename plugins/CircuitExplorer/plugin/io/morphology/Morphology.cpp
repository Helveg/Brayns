...
Morphology::Morphology(const std::string& path,
                       const std::unordered_set<SectionType>& sections)
 : _morphologyPath(path)
 , _soma(nullptr)
{
    const bool loadSoma = sections.find(SectionType::SOMA) != sections.end();
    const bool loadAxon = sections.find(SectionType::AXON) != sections.end();
    const bool loadDend = sections.find(SectionType::DENDRITE) != sections.end();
    const bool loadADen = sections.find(SectionType::APICAL_DENDRITE) != sections.end();
