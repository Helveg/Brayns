...
void SDFNeuronBuilder::_buildImpl(const NeuronMorphology& m)
{
    InternalBuilder builder (m);

    _sdfGeometries = std::move(builder._geometry);

    _sdfData = std::make_shared<SDFSharedData>();
    _sdfData->neighbours.resize(_sdfGeometries.size());
...
MorphologyInstancePtr
SDFNeuronBuilder::_instantiateImpl(const brayns::Vector3f& tr,
                                   const brayns::Quaternion& rot) const
{
    ...
    return std::make_unique<SDFNeuronInstance>(std::move(transformedSDF), _sdfData);
}
