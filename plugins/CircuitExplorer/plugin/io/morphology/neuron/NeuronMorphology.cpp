...
std::vector<NeuronMorphology::Section> readNeurites(const morphio::Morphology& m,
                                                    const bool axon,
                                                    const bool dend,
                                                    const bool adend)
{
...
NeuronMorphology::NeuronMorphology(const std::string& path, const NeuronSection section)
 : _morphologyPath(path)
 , _soma(nullptr)
{
    if(section == NeuronSection::NONE)
        throw std::invalid_argument("NeuronMorphology: Request to load 'none' not allowed");

    // If only soma requested, do not load the morphology file
    if(section == NeuronSection::SOMA)
        _soma = std::make_unique<Soma>(brayns::Vector3f(0.f, 0.f, 0.f), 1.f);
    else
    {
        const bool loadSoma = (NeuronSection::SOMA & section) == NeuronSection::SOMA;
        const bool loadAxon = (NeuronSection::AXON & section) == NeuronSection::AXON;
        const bool loadDend = (NeuronSection::DENDRITE & section) == NeuronSection::DENDRITE;
        const bool loadADen = (NeuronSection::APICAL_DENDRITE & section)
...
std::vector<NeuronMorphology::Section*>
NeuronMorphology::sectionChildren(const Section& parent) noexcept
{
...
std::vector<const NeuronMorphology::Section*>
NeuronMorphology::sectionChildren(const Section& parent) const noexcept
{
