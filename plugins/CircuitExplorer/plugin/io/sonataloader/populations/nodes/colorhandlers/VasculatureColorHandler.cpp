...
void
VasculatureColorHandler::_updateColorByIdImpl(const std::map<uint64_t, brayns::Vector4f>& colorMap)
{
    if(!colorMap.empty())
    {
...
void VasculatureColorHandler::_updateColorImpl(const std::string&, const ColorVariables& variables)
{
    if(!variables.empty())
    {
        ...
        {
            const auto sectionType = EnumWrapper<VasculatureSection>().fromString(entry.first);
