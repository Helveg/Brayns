...
inline const std::vector<std::pair<const char*, const char*>>& __methodAttribMapping() noexcept
{
...
inline std::vector<std::string> __getMethodsValues(const bbp::sonata::CircuitConfig& config,
                                              ...
                                              std::vector<ElementMaterialMap::Ptr>&& elements)
{
    ...
    std::sort(tempBuf.begin(), tempBuf.end(), [](const SynapseMaterialInfo& a,
                                                 const SynapseMaterialInfo& b)
    {
        return a.id < b.id;
    });
...
CommonEdgeColorHandler::_getMethodVariablesImpl(const std::string& method) const
{
    const auto values = __getMethodsValues(_config, _nodePopulation, method, _nodeIds);
...
void CommonEdgeColorHandler::_updateSingleColorImpl(const brayns::Vector4f& color)
{
    for(const auto& group : _elements)
    {
        for(const auto matId : group)
