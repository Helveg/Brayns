...
void __checkEdgeParameters(const bbp::sonata::EdgePopulation& population,
...
inline std::vector<brayns::Vector3f> __loadSurfacePos(const bbp::sonata::EdgePopulation& pops,
                                                      const bbp::sonata::Selection& selection,
                                                      const char* attribX,
                                                      const char* attribY,
                                                      const char* attribZ)
{
    __checkEdgeParameters(pops, {attribX, attribY, attribZ});
    ...
    if(surfaPosXs.size() != surfaPosYs.size() || surfaPosXs.size() != surfaPosZs.size())
        throw std::runtime_error("Edge population '" + pops.name() + "' surface position "
...
std::vector<brayns::Vector3f> SonataSynapses::getAfferentSurfacePos(const Edges& population,
                                                                    const Selection& selection)
{
    return __loadSurfacePos(population,
                            selection,
                            attribAffSurfPosiX,
                            attribAffSurfPosiY,
                            attribAffSurfPosiZ);
...
std::vector<float> SonataSynapses::getAfferentSectionDistances(const Edges& population,
                                                               const Selection& selection)
{
    __checkEdgeParameters(population, {attribAffSegmentPos});
...
std::vector<float> SonataSynapses::getEfferentSectionDistances(const Edges& population,
                                                               const Selection& selection)
{
    __checkEdgeParameters(population, {attribEffSegmentPos});
...
std::vector<float> SonataSynapses::getEfferentAstrocyteSectionDistances(const Edges& population,
                                                                        const Selection& selection)
{
    __checkEdgeParameters(population, {attribAstroSectionPos});
...
{
    __checkEdgeParameters(population, {attribEndFeetId});
