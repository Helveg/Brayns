...
std::vector<brayns::Vector3f> __getPoints(const bbp::sonata::NodePopulation& nodes,
                                          const bbp::sonata::Selection& selection,
                                          const char* ax,
                                          const char* ay,
                                          const char* az)
{
    __checkAttributes(nodes, {ax, ay, az});
...
std::vector<float> __getRadii(const bbp::sonata::NodePopulation& nodes,
                              const bbp::sonata::Selection& selection,
                              const char* ar)
{
    __checkAttributes(nodes, {ar});
    auto radii = nodes.getAttribute<float>(ar, selection);
    std::transform(radii.begin(), radii.end(), radii.begin(), [](const float diameter)
    {
        return diameter * .5f;
    });
    return radii;
...
SonataVasculature::getSegmentStartPoints(const Nodes& nodes, const Selection& selection)
{
    return __getPoints(nodes, selection, attribStartX, attribStartY, attribStartZ);
}

std::vector<brayns::Vector3f>
SonataVasculature::getSegmentEndPoints(const Nodes& nodes, const Selection& selection)
{
    return __getPoints(nodes, selection, attribEndX, attribEndY, attribEndZ);
}

std::vector<float>
SonataVasculature::getSegmentStartRadii(const Nodes& nodes, const Selection& selection)
{
    return __getRadii(nodes, selection, attribStartDiameter);
}

std::vector<float>
SonataVasculature::getSegmentEndRadii(const Nodes& nodes, const Selection& selection)
{
...
{
    __checkAttributes(nodes, {attribType});
