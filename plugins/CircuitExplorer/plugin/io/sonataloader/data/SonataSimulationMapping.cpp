...
inline auto __computeMapping(const HighFive::Group& reportPop,
                             ...
                             const std::vector<bbp::sonata::NodeID>& nodeIds)
{
    ...
    for (const auto nodeId : nodeIds)
    {
        ...
        elementIdsData.select({it->second.first}, {it->second.second - it->second.first})
...
SonataSimulationMapping::getCompartmentNodes(const std::string& reportPath,
...
SonataSimulationMapping::getCompartmentMapping(const std::string& reportPath,
                                               const std::string& population,
                                               const std::vector<bbp::sonata::NodeID>& nodeIds)
{
    ...
    for (size_t i = 0; i < reportNodeIds.size(); ++i)
        nodePointers.emplace(reportNodeIds[i], std::make_pair(indexPointers[i],
                                                              indexPointers[i + 1]));
