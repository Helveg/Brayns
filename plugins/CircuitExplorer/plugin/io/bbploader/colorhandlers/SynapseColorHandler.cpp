...
namespace bbploader
{
SynapseColorHandler::SynapseColorHandler(brayns::ModelDescriptor* model)
 : CircuitColorHandler(model)
{
}

void SynapseColorHandler::_setElementsImpl(const std::vector<uint64_t>& ids,
...
std::vector<std::string> SynapseColorHandler::_getMethodsImpl() const
{
    return {};
}
...
void SynapseColorHandler::_updateSingleColorImpl(const brayns::Vector4f& color)
{
    for(const auto& materials : _synapseMatIds)
    {
        for(const auto mat : materials)
