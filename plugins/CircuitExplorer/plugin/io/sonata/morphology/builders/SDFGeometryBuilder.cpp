...
#include "SDFGeometryBuilder.h"

#include "../MorphologyPipeline.h"
#include "../instances/SDFMorphologyInstance.h"
...
public:
    Registerer()
    {
        MorphologyPipeline::registerBuilder<SDFGeometryBuilder>("smooth");
    }
};
...
struct MorphologyHierarchy
{
    std::unordered_map<int32_t, std::vector<size_t>> _sections;
};

}

class SDFGeometryBuilder::InternalBuilder
{
public:
    InternalBuilder(const Morphology& m)
     : m(m)
    {
        // Soma
        if(m.hasSoma())
        ...
        for (const auto& section : m.sections())
        {
            ...
            if(section.samples.size() > 1)
            {
                for (size_t i = 1; i < section.samples.size(); ++i)
                {
                    ...
                    if (s1 != s2)
                        sectionGeoms.push_back(_addSegment(p2, r2,
                                                           p1, r1,
    ...
    std::vector<MorphologySection> _sectionTypeMap;
private:
    const Morphology& m;
    ...
    void _addSomaGeometry()
    {
        ...
        if(!soma.children.empty())
        {
            ...
            for (const auto child : soma.children)
            {
                ...
                auto geometry = brayns::createSDFConePillSigmoid(soma.center, samplePos,
                                                                 soma.radius, sampleRadius);
                const size_t geomIdx = _addSDFGeometry(std::move(geometry),
                                                       -1,
                                                       MorphologySection::SOMA);
...
void SDFGeometryBuilder::build(const Morphology& m)
{
    _somaPos = m.hasSoma()? m.soma().center : brayns::Vector3f(0.f, 0.f, 0.f);

    InternalBuilder builder (m);

    _sdfGeometries = std::move(builder._geometry);
    _sdfNeighbours.resize(_sdfGeometries.size());
    for(size_t i = 0; i < _sdfGeometries.size(); ++i)
    {
        // Copy set neighbours into final vector neighbour
        const auto& tn = builder._neighbours[i];
        _sdfNeighbours[i] = std::vector<size_t>(tn.begin(), tn.end());
    }
    _sectionMap = std::move(builder._sectionMap);
    _sdfSectionTypes = std::move(builder._sectionTypeMap);
}

std::unique_ptr<MorphologyInstance>
SDFGeometryBuilder::instantiate(const brayns::Vector3f& tr, const brayns::Quaternion& rot) const
{
    auto transformedSDF = _sdfGeometries;
    for(auto& geometry : transformedSDF)
    {
        geometry.p0 = tr + _somaPos + (rot * (geometry.p0 - _somaPos));
        geometry.p1 = tr + _somaPos + (rot * (geometry.p1 - _somaPos));

    }

    return std::make_unique<SDFMorphologyInstance>(transformedSDF,
                                                   _sdfNeighbours,
                                                   _sdfSectionTypes,
                                                   _sectionMap);
}
