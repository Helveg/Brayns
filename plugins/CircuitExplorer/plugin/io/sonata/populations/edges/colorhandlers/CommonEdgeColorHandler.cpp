...
inline const std::vector<std::pair<const char*, const char*>>& methodAttribMapping() noexcept
{
    static const std::vector<std::pair<const char*, const char*>> possibleMethods =
    {
...
inline const char* getAttribForMethod(const std::string& method)
{
    ...
    throw std::invalid_argument("CommonEdgeColorHandler: Unknown method '" + method + "'");
}

inline std::vector<std::string> getMethodsValues(const bbp::sonata::CircuitConfig& config,
                                                 ...
                                                 const std::vector<uint64_t>& nodeIds)
{
    ...
    const auto nodeSelection = bbp::sonata::Selection::fromValues(uniqueNodes.begin(),
                                                                  uniqueNodes.end());
    return nodes.getAttribute<std::string>(attrib, nodeSelection);
}
}

CommonEdgeColorHandler::CommonEdgeColorHandler(brayns::ModelDescriptor* model,
                                              ...
                                              std::vector<ElementMaterialMap::Ptr>&& elements)
{
    ...
    // we dont know which edges has been added to the geometry, nor the percentage used.

    // Flatten edges
    std::vector<EdgeMaterialInfo> tempBuf;
    for(auto& element : elements)
    {
        auto& semm = static_cast<SurfaceEdgeMaterialMap&>(*element.get());
        tempBuf.insert(tempBuf.end(), semm.materials.begin(), semm.materials.end());
    }

    std::sort(tempBuf.begin(), tempBuf.end(), [](const EdgeMaterialInfo& a,
                                                 const EdgeMaterialInfo& b)
    {
        return a.id < b.id;
    });
    ...
    // Get opposite node Ids depending wether its afferent or efferent loaded population
    const auto selection = bbp::sonata::Selection::fromValues(edgeIds);
    const auto edges = _config.getEdgePopulation(_population);
    const auto nodeIds = _afferent ? SonataSynapses::getSourceNodes(edges, selection)
                                   : SonataSynapses::getTargetNodes(edges, selection);

    // Group them
    std::map<uint64_t, std::vector<EdgeMaterialInfo>> groupedEdges;
...
std::vector<std::string> CommonEdgeColorHandler::_getMethodsImpl()
{
...
std::vector<std::string> CommonEdgeColorHandler::_getMethodVariablesImpl(const std::string& method)
{
...
void
CommonEdgeColorHandler::_updateColorByIdImpl(const std::map<uint64_t, brayns::Vector3f>& colorMap)
{
    ColorRoulette r;
...
void CommonEdgeColorHandler::_updateSingleColorImpl(const brayns::Vector3f& color)
{
    for(const auto& group : _elements)
    {
        for(const auto matId : group)
...
void CommonEdgeColorHandler::_updateColorImpl(const std::string& method, const ColorVariables& variables)
{
    const auto values = ::getMethodsValues(_config, _nodePopulation, method, _nodeIds);
