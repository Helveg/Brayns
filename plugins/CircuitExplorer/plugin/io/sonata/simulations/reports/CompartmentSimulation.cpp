...
#include "../handlers/SonataReportHandler.h"
...
public:
    Registerer()
    {
        SonataSimulationFactory::instance()
                .registerSimulation<CompartmentSimulation>(SimulationType::REPORT);
    }
};
...
std::vector<CellMapping> CompartmentSimulation::loadMapping(const bbp::sonata::Selection& s) const
{
    ...
    std::vector<CellMapping> mapping (sortedCompartmentsSize.size());
...
CompartmentSimulation::createSimulationHandler(const bbp::sonata::Selection& s) const
{
