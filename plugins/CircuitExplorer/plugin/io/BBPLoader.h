 ...
 */
class BBPLoader : public brayns::Loader
{
public:
    /**
     * @brief Constructor
     */
    BBPLoader(brayns::Scene& scene, CircuitColorManager& colorManager);
    ...
    /**
     * @brief importFromBlueConfig imports a neuronal circuit from a BlueConfig file.
     *        NOTE: Temporary utility function so SonataNGVLoader can call this loader for
     *        each population.
     *        TODO: Remove once NGV Project has switched to SONATA format...
     */
    std::vector<brayns::ModelDescriptorPtr>
    importFromBlueConfig(const std::string& path,
                         const brayns::LoaderProgress& callback,
