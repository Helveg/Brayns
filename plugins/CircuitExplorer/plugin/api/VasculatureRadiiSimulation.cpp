...
void VasculatureRadiiSimulation::registerModel(brayns::ModelDescriptor* model)
{
    ...
    auto radiiHandler = dynamic_cast<sonataloader::VasculatureRadiiHandler*>(handler.get());
    ...
    for(auto& sdfGeometry : model->getModel().getSDFGeometryData().geometries)
    {
        if(sdfGeometry.userData > maxIndex)
            throw std::runtime_error("VasculatureRadiiSimulation: Vasculature geometry is "
                                     "mapped beyond frame size");
    }

    _vasculatureModels.push_back({model, std::numeric_limits<uint32_t>::max()});
}

void VasculatureRadiiSimulation::unregisterModel(size_t modelId)
{
    auto it = std::find_if(_vasculatureModels.begin(),
                           _vasculatureModels.end(),
