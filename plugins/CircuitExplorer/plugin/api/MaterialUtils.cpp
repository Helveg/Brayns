...
size_t CircuitExplorerMaterial::create(brayns::Model& model, const brayns::Vector3f& color)
{
...
void CircuitExplorerMaterial::setSimulationColorEnabled(brayns::Model& model, const bool value)
{
    for(auto& matEntry : model.getMaterials())
    {
        if(!matEntry.second->hasProperty(MATERIAL_PROPERTY_CAST_USER_DATA))
            continue;
        matEntry.second->updateProperty(MATERIAL_PROPERTY_CAST_USER_DATA, value);
        matEntry.second->markModified();
        matEntry.second->commit();
    }
}
...
const brayns::PropertyMap& CircuitExplorerMaterial::_getExtraAttributes() noexcept
{
    ...
    if(extra.empty())
    {
        extra.setProperty({MATERIAL_PROPERTY_CAST_USER_DATA, false});
        extra.setProperty({MATERIAL_PROPERTY_SHADING_MODE,
                            static_cast<int32_t>(MaterialShadingMode::diffuse)});
        extra.setProperty({MATERIAL_PROPERTY_CLIPPING_MODE,
                            static_cast<int>(MaterialClippingMode::no_clipping)});
        extra.setProperty({MATERIAL_PROPERTY_USER_PARAMETER, 1.0});
    }
    return extra;
}
